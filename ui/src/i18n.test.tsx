...
function Probe(){const{locale,setLocale,t}=useI18n();return <><span>{locale}</span><strong>{t('dashboard.newTask')}</strong><span>{t('nav.costs')}</span><code>Oberth v0.1.0-alpha.5</code><button onClick={()=>setLocale('es')}>switch</button></>}
...
describe('i18n',()=>{
  ...
  it('defaults to English and persists Spanish',()=>{
    ...
    expect(screen.getByText('Oberth v0.1.0-alpha.5')).toBeInTheDocument()
