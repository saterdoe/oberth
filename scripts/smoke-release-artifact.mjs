...
const cli = path.join(packageDir, `oberth${suffix}`)
const server = path.join(packageDir, `oberth-server${suffix}`)
...
for (const name of [`oberth${suffix}`, `oberth-server${suffix}`, 'LICENSE', 'NOTICE', 'THIRD_PARTY_NOTICES.md', 'SHA256SUMS', 'sbom.spdx.json']) {
  requireFile(name)
}
