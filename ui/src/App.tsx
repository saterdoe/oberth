  ...
  return <section className="panel session-panel"><div className="session-main">
    ...
    <div className="session-body"><div className="session-left">
      ...
      {Object.keys(bundle).length>0&&<div className="run-evidence"><Label>Resultado</Label>
        ...
        {!readOnly&&(diff.length>0?<div id="run-diff"><Label>Cambios para revisar</Label>{diff.map(file=><details className="diff-file" key={file.path} open><summary>{file.status} · {file.path}<button onClick={event=>{event.preventDefault();const preferred=launchers.find(launcher=>launcher.available&&launcher.id!=='folder')||launchers.find(launcher=>launcher.available);if(preferred)void openIDE(preferred.id,file.path)}}>Abrir archivo</button></summary><DiffPreview content={file.content}/></details>)}</div>:<CompactEmpty text="Sin cambios de archivos" detail="Puede ser una ejecución sólo de verificación."/>)}
