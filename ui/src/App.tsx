...
function SessionPanel({session,run,project,launchers,stream,onChanged}:{session?:Session;run?:Run;project?:Project;launchers:Launcher[];stream?:string;onChanged:()=>Promise<void>}) {
  ...
  const blockers=[verification!=='passed'?`Verificación: ${verification}`:'',!diffHash?'Falta ligar el diff verificado':'',promotionReadiness?.ready===false?(promotionReadiness.reason||'El checkout no está listo'):''].filter(Boolean)
  ...
  return <section className="panel session-panel"><div className="session-main">
    <div className="session-head"><div className="session-title" data-no-translate>{session.task_description||session.task_type}</div><div className="session-meta"><span data-no-translate>{project?.name||t('session.repoUnavailable')} · {runDetail?.base_repository||run?.base_repository||project?.path||t('session.pathUnavailable')}</span>{!readOnly&&<span data-no-translate>base {(runDetail?.base_commit||run?.base_commit)?.slice(0,8)||t('session.basePending')} · worktree {runDetail?.branch||run?.branch||t('session.worktreePending')}</span>}<span>{elapsed(session.started_at)}</span></div><div className="repository-safety">{readOnly?t('session.readOnly'):session.status==='completed'?<><Check size={11}/> {t('session.appliedCheckout')}</>:t('session.isolated')}</div></div>
      ...
      {!readOnly&&approvalTurn&&<div className="task-toolbar session-decision"><button disabled={Boolean(actionBusy)} onClick={()=>void approveAndRetry()}>{actionBusy==='approval'?'Aplicando aprobación…':'Aprobar riesgo para este proyecto y reintentar'}</button></div>}
      {!readOnly&&run?.state==='review'&&!resolvedOutcome&&<div className="review-actions session-decision"><div className="review-head"><strong>Revisá los cambios y decidí</strong><span>{verification==='passed'?'QA aprobado':'QA pendiente'} · {reviewedCount}/{diff.length} archivos vistos · {warnings.length} advertencias</span></div><div className="task-toolbar"><button disabled={Boolean(actionBusy)||!promotable} title={promotable?'QA aprobado y checkout listo para aplicar':blockers.join(' · ')||'Comprobando requisitos'} onClick={()=>void decide('accepted')}>{actionBusy==='accepted'?'Aplicando cambios…':promotable?`Aplicar cambios a ${project?.name||'repositorio'}`:'Aplicación bloqueada'}</button><button className="danger" disabled={Boolean(actionBusy)} onClick={()=>void decide('rejected')}>{actionBusy==='rejected'?'Descartando…':'Descartar cambios'}</button></div>{blockers.length>0&&<div className="promotion-readiness" role="status"><strong>Antes de aplicar:</strong>{blockers.map(item=><span key={item}>{item}</span>)}</div>}<label>¿Hace falta corregir algo?<textarea value={correction} onChange={event=>setCorrection(event.target.value)} placeholder="Indicá qué debe corregir el agente; se iniciará un nuevo intento."/></label><button className="secondary-review-action" disabled={Boolean(actionBusy)||!correction.trim()} onClick={()=>void requestCorrection()}>{actionBusy==='corrected'?'Creando nuevo intento…':'Pedir corrección'}</button></div>}
    ...
    <div className="session-body"><div className="session-left">
      {!readOnly&&<div className={`changes-callout ${diff.length?'has-changes':''}`}><Label>{t('session.agentChanges')}</Label>{diff.length?<><strong>{t('session.filesReady',{count:diff.length})}</strong><span data-no-translate>{diff.map(file=>file.path).join(' · ')}</span><a href="#run-diff">{t('session.viewDiff')}</a></>:<><strong>{run?.state==='blocked'||blocked?'Ejecución bloqueada antes de generar cambios':'Sin diff disponible'}</strong><span>{run?.state==='running'?'El diff aparecerá aquí durante la ejecución.':run?.state==='blocked'||blocked?'No hay nada para aplicar. Revisá la causa y la próxima acción en el bloque de error antes de reintentar.':'Este run no produjo cambios de archivos; revisá las advertencias y el timeline.'}</span></>}</div>}
      ...
      {Object.keys(bundle).length>0&&<div className="run-evidence"><Label>Resultado</Label>
        {readOnly&&answer&&<article className="answer-card"><div className="answer-card-title">Respuesta</div><div className="answer-content">{answer}</div></article>}
        {readOnly&&answer&&<div className="conversation-next"><label>Continuar esta conversación<textarea value={followUp} onChange={event=>setFollowUp(event.target.value)} placeholder="Hacé otra pregunta o describí el cambio que querés discutir."/></label><div className="task-toolbar"><button disabled={Boolean(actionBusy)||!followUp.trim()} onClick={()=>void continueConversation('query',followUp)}>{actionBusy==='conversation-query'?'Consultando…':'Hacer otra pregunta'}</button><button disabled={Boolean(actionBusy)||!followUp.trim()} onClick={()=>void continueConversation('plan',followUp)}>{actionBusy==='conversation-plan'?'Preparando plan…':'Proponer plan de implementación'}</button>{interactionMode==='plan'&&<button className="primary" disabled={Boolean(actionBusy)} onClick={()=>void continueConversation('implementation',`Implement the approved plan exactly as discussed:\n\n${answer}`)}>{actionBusy==='conversation-implementation'?'Implementando…':'Aprobar e implementar este plan'}</button>}</div><small>Las preguntas y los planes son de sólo lectura. Oberth sólo modifica archivos después de aprobar el plan.</small></div>}
        ...
        {!readOnly&&(diff.length>0?<div id="run-diff"><Label>Cambios para revisar · {reviewedCount}/{diff.length} vistos</Label>{diff.map(file=><details className={`diff-file ${viewedFiles.has(file.path)?'viewed':''}`} key={file.path} onToggle={event=>{if(event.currentTarget.open)setViewedFiles(current=>new Set(current).add(file.path))}}><summary>{viewedFiles.has(file.path)?'✓ visto':'○ pendiente'} · {file.status} · {file.path}<button onClick={event=>{event.preventDefault();const preferred=launchers.find(launcher=>launcher.available&&launcher.id!=='folder')||launchers.find(launcher=>launcher.available);if(preferred)void openIDE(preferred.id,file.path)}}>Abrir archivo</button></summary><DiffPreview content={file.content}/></details>)}</div>:<CompactEmpty text="Sin cambios de archivos" detail="Puede ser una ejecución sólo de verificación."/>)}
...
function ContextHelp({topic,commit}:{topic:'recovery'|'configuration';commit?:string}){
  ...
  const copy={recovery:{en:['Recovery and promotion','Cause: this run needs attention. Impact: the main checkout remains unchanged. Next: review the evidence, preserve the worktree and retry safely.','Recovery guide'],es:['Recuperación y promoción','Causa: este run necesita atención. Impacto: el checkout principal sigue intacto. Próximo paso: revisá la evidencia, conservá el worktree y reintentá de forma segura.','Guía de recuperación'],doc:'RELEASE_RUNBOOK.md'},configuration:{en:['Providers, indexing and memory','Providers generate results; indexes retrieve local context; only approved evidence becomes memory. Failures preserve the previous configuration.','Configuration guide'],es:['Proveedores, índices y memoria','Los proveedores generan resultados; los índices recuperan contexto local; sólo la evidencia aprobada se vuelve memoria. Las fallas conservan la configuración anterior.','Guía de configuración'],doc:'QUICKSTART.md'}}[topic]
  const text=locale==='es'?copy.es:copy.en
  return <aside className={`context-help context-help-${topic}`} aria-label={text[0]}><strong>{text[0]}</strong><span>{text[1]}</span><a href={`https://github.com/saterdoe/oberth/blob/${encodeURIComponent(commit||'main')}/docs/${copy.doc}`} target="_blank" rel="noreferrer">{text[2]} ↗</a></aside>
}
